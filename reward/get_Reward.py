...
class get_Reward(object):
    def __init__(self, batch_num, maxlen, dim, inputdata, sl, su, lambda1_upper, score_type='BIC', reg_type='LR', l1_graph_reg=0.0):
    ...
    def calculate_reward_single_graph(self, graph_batch, tgraph, lambda1, lambda2, lambda3):
        ...
        if self.score_type == 'BIC':
            BIC = np.log(np.sum(RSS_ls)/self.n_samples+1e-8) + np.sum(graph_batch.numpy())*self.bic_penalty/self.maxlen 
        elif self.score_type == 'BIC_different_var':
            BIC = np.sum(np.log(np.array(RSS_ls)/self.n_samples+1e-8)) + np.sum(graph_batch.numpy())*self.bic_penalty

        score = self.score_transform(BIC)
        cycness = np.trace(matrix_exponential(np.array(graph_batch.numpy())))- self.maxlen
